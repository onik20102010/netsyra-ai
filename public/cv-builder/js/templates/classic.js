...
window.CVTemplates.classic = {
  ...
  render(data) {
    ...
    return `
      <div style="font-family:'Georgia','Times New Roman',serif;padding:40px;color:#222;line-height:1.6;">
        ...
        ${tplListSection('Projects', data.projects, proj => `
          <div style="margin-bottom:8px;">
            ...
            ${proj.github || proj.liveUrl ? `<p style="font-size:11px;color:#555;">${[proj.github, proj.liveUrl].filter(Boolean).map(escapeHTML).join(' | ')}</p>` : ''}
          </div>
        `)}
        ${tplListSection('Certifications', data.certifications, c => `
          <div style="margin-bottom:6px;">
            <strong>${escapeHTML(c.name)}</strong> — ${escapeHTML(c.organization)}
            <span style="color:#777;font-size:11px;"> (${formatDate(c.issueDate)})</span>
        ...
        ${tplListSection('Awards', data.awards, a => `
          <div style="margin-bottom:6px;">
            ...
            ${a.description ? `<p style="font-size:12px;margin:2px 0;">${escapeHTML(a.description)}</p>` : ''}
          </div>
        `)}
        ...
        ${tplListSection('Publications', data.publications, pub => `
          <div style="margin-bottom:6px;">
            ...
            ${pub.url ? `<br><span style="font-size:11px;">${escapeHTML(pub.url)}</span>` : ''}
          </div>
        `)}
        ${tplListSection('Conferences', data.conferences, c => `
          <div style="margin-bottom:6px;">
            ...
            ${c.description ? `<p style="font-size:12px;margin:2px 0;">${escapeHTML(c.description)}</p>` : ''}
          </div>
        `)}
...
function renderSocialLinks(social) {
  ...
  return `
    ...
    <p style="font-size:12px;">${links.map(([k, v]) => `${escapeHTML(k)}: ${escapeHTML(v)}`).join(' | ')}</p>
  `;
}
    ...
    return `
      ...
      ${items.map(item => `
        <div style="margin-bottom:6px;">
          ...
          ${item.description ? `<p style="font-size:12px;margin:2px 0;">${escapeHTML(item.description)}</p>` : ''}
        </div>
      `).join('')}
    `;
  }).join('');
